...
dbLoadRecords("../../db/PSHV/PSHVInitialize.vdb","Group=DEFL")

###############################Read and write analog values for PSHV and Septum Temp
dbLoadRecords("../../db/PSHV/PSHVAnalog.vdb","Group=DEFL")

#############################PSHV On/OFF
dbLoadRecords("../../db/PSHV/PSHVOnOff.vdb","Group=DEFL")

#############################PSHV Check Interlocks
dbLoadRecords("../../db/PSHV/PSHVCondition.vdb","Group=DEFL")

#############################PSHV Check Interlocks
dbLoadRecords("../../db/PSHV/PSHVBeamInterlockCondition.vdb","Group=DEFL")

#############################Define PSHV Control Parameters
dbLoadRecords("../../db/PSHV/PSHVParamSet.vdb","Group=DEFL, Param=Volt,NoAttach=1,Mult=.0001,EGU=KV,PREC=2,PWZ=.5,DRVH=60,DRVL=0")
...
dbLoadRecords("../../db/Source/SourceInitialize.vdb","Group=Source")

###############################Read and write analog values for PSARC AND Gas Flow Meter
dbLoadRecords("../../db/Source/SourceAnalog.vdb","Group=Source")

###############################Starts HB Processes, and writes HB
dbLoadRecords("../../db/Source/SourceControlSeq.vdb","Group=Source")

###############################Define Control Parameters for Gas Flow Meter
dbLoadRecords("../../db/Source/SourceParamSet.vdb","Group=Source, Device=Gas, Param=Flow, NoAttach=1, Mult=.00015, EGU=CCM, PREC=2, PWZ=0.1, DRVH=10, DRVL=0")

#############################SOURCE On/OFF PSARC Only 
dbLoadRecords("../../db/Source/SourceOnOff.vdb","Group=Source")

#############################PSARC Check Interlocks for PSARC and Gas Manifold
dbLoadRecords("../../db/Source/SourceCondition.vdb","Group=Source, GasValveDRVL=3.1, GasValveDRVH=3.7")

#############################PSARC Record N=1/2 Cathode Histories
dbLoadRecords("../../db/Source/SourceCathodeMonitor.vdb","Group=Source")

############################Select Particle Type
dbLoadRecords("../../db/Source/SourceGasSelect.vdb","SubSys=Gas, Device=Type")

############################PID Control for Gas Manifold only
dbLoadRecords("../../db/Source/SourcePIDControl.vdb","Group=Source, KP=0.035, KD=0.00, KI=0.00")

##############################Source System Reset
dbLoadRecords("../../db/Source/SourceSystemReset.vdb","Group=Source")

##Load stdby 1 to stby 2 transition database
dbLoadRecords("../../db/Source/SourceSystemSB1SB2.vdb","Group=Source")
...
dbLoadRecords("../../db/ISM/ISMOnOff.vdb", "SubSys=ISM, Device=Output, Param=Curr, Group=ISM")
dbLoadRecords("../../db/ISM/ISMInitialize.vdb", "Group=ISM")
dbLoadRecords("../../db/ISM/ISMInitializeSupply.vdb", "SubSys=ISM, Device=Output, R=1, AsynPort=ISMGateway")
dbLoadRecords("../../db/ISM/ISMCondition.vdb","SubSys=ISM, Device=Output, Param=Curr, Group=ISM")
...
dbLoadRecords("../../db/ISM/ISMAnalog.vdb", "SubSys=ISM, Device=Output, Param=Curr,Group=ISM, R=1, AsynPort=ISMGateway, Prec=2")
dbLoadRecords("../../db/ISM/ISMControlSeq.vdb", "Group=ISM")
